...
```{r}
# Redcap
selfreport <- read_csv("vdap_cut.csv")

# Gorilla Visual Search Task
cog1 <- read_csv("vdap_vs_final.csv")

# Gorilla Flanker Task
cog2 <- read_csv("gor_final.csv")
...
```{r}
# First impute missing values with numerical medians or categorical modes
data_merge$age[is.na(data_merge$age)] <- median(data_merge$age, na.rm=TRUE)
data_merge$sex[is.na(data_merge$sex)] <- max(data_merge$sex, na.rm = TRUE)
data_merge$ethnicity[is.na(data_merge$ethnicity)] <- median(data_merge$ethnicity, na.rm = TRUE)
data_merge$pre_race[is.na(data_merge$pre_race)] <- median(data_merge$pre_race, na.rm = TRUE)
data_merge$area_type[is.na(data_merge$area_type)] <- median(data_merge$area_type, na.rm = TRUE)
data_merge$edu_2[is.na(data_merge$edu_2)] <- median(data_merge$edu_2, na.rm = TRUE)
data_merge$alcohol_status[is.na(data_merge$alcohol_status)] <- median(data_merge$alcohol_status, na.rm = TRUE)
data_merge$psychiatric_hist_status[is.na(data_merge$psychiatric_hist_status)] <- min(data_merge$psychiatric_hist_status, na.rm = TRUE)
data_merge$verify_veteran[is.na(data_merge$verify_veteran)] <- max(data_merge$verify_veteran, na.rm=TRUE)
data_merge$nicotine[is.na(data_merge$nicotine)] <- median(data_merge$nicotine, na.rm=TRUE)
data_merge$cannabis[is.na(data_merge$cannabis)] <- median(data_merge$cannabis, na.rm=TRUE)
